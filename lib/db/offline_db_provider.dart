...
class OfflineDbProvider {
  ...
  initDB() async {
    ...
    var maxMigratedDbVersion = DbMigrator.migration.keys.reduce(max);    
    ...
    _database = await openDatabase(
      ...
      onCreate: (Database db, int _) async {
        DbMigrator.migration.keys.toList()
        ..sort()
        ..forEach((k) async {
          var script = DbMigrator.migration[k];
      ...
      onUpgrade: (Database db, int _, int __) async {
        ...
        var upgradeScripts = new Map.fromIterable(
          DbMigrator.migration.keys.where((k) => k > curdDbVersion), 
          key: (k) => k, value: (k) => DbMigrator.migration[k]
        );
